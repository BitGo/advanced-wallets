...
export async function eddsaInitialize(
  req: AdvancedWalletManagerApiSpecRouteRequest<'v1.mpc.key.initialize', 'post'>,
) {
  ...
  // construct encrypted payload. awm receives back this payload in finalize since it can't keep it in memory
  const payload = {
