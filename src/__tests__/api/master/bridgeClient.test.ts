...
import 'should';
import { BridgeClient } from '../../../masterBitgoExpress/clients/bridgeClient';
...
describe('BridgeClient', () => {
  ...
  before(() => {
    ...
    client = new BridgeClient(BASE_URL, TIMEOUT);
  });
  ...
  describe('constructor', () => {
    it('throws when url is empty', () => {
      (() => new BridgeClient('', TIMEOUT)).should.throw('BridgeClient: awmAsyncUrl is required');
    });

    it('strips trailing slash from url', () => {
      const c = new BridgeClient(`${BASE_URL}/`, TIMEOUT);
      nock(BASE_URL).get('/health').reply(200, { ok: true });
  ...
  describe('health()', () => {
    it('GETs /health and returns body', async () => {
      const n = nock(BASE_URL).get('/health').reply(200, { ok: true });

      const result = await client.health();

      (result as any).should.have.property('ok', true);
      n.done();
    });
  });
