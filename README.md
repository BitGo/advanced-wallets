...
- `ADVANCED_WALLET_MANAGER_URL` - Advanced Wallet Manager server URL (required)
- `ADVANCED_WALLET_MANAGER_CERT` - Path to Advanced Wallet Manager server certificate (required)
...
```bash
curl -k -X POST https://localhost:3081/ping/awm
```
...
export TLS_KEY_PATH=/secure/path/awm.key
export TLS_CERT_PATH=/secure/path/awm.crt
export MTLS_REQUEST_CERT=true
...
export ADVANCED_WALLET_MANAGER_CERT=/secure/path/awm.crt
export MTLS_REQUEST_CERT=true
export ALLOW_SELF_SIGNED=false
npm start
```
...
podman run -d \
  ...
  -e BIND=0.0.0.0 \
  -e TLS_MODE=mtls \
  -e TLS_KEY_PATH=/app/certs/awm-key.pem \
  -e TLS_CERT_PATH=/app/certs/awm-cert.pem \
...
podman run -d \
  ...
  -e ADVANCED_WALLET_MANAGER_CERT=/app/certs/awm-cert.pem \
  -e ALLOW_SELF_SIGNED=true \
  bitgo-onprem-express
...
curl -k --cert certs/test-ssl-cert.pem --key certs/awm-key.pem -X POST https://localhost:3080/ping
...
```bash
DEBUG_NAMESPACE=awm:*,master:* npm run start
```
